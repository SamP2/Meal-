...
const FILTERS = ['Nearby', 'Top Rated', 'Pure Veg', 'Budget'];
...
export default function DiscoverScreen({ navigation }: any) {
  ...
  const applyFilter = (list: Mess[], filter: string, q: string) => {
    let result = [...list];
    if (q) result = result.filter(m => m.name.toLowerCase().includes(q.toLowerCase()));
    if (filter === 'Top Rated') result = result.sort((a, b) => b.rating - a.rating);
    if (filter === 'Pure Veg') result = result.filter(m => m.is_veg);
    if (filter === 'Budget') result = result.filter(m => m.price_range?.includes('₹'));
    setFiltered(result);
  };

  const handleSearch = (text: string) => {
    setSearch(text);
    applyFilter(messes, activeFilter, text);
  };

  const handleFilter = (f: string) => {
    setActiveFilter(f);
    applyFilter(messes, f, search);
  };
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.surface }]}>
        <View>
          ...
          <Text style={[styles.locationName, { color: colors.primary }]}>Near Campus</Text>
        </View>
        ...
        ListHeaderComponent={() => (
          <View style={styles.listHeader}>
            ...
            <View style={styles.sectionHeader}>
              <Text style={[styles.sectionTitle, { color: colors.text }]}>Curated Freshness</Text>
            ...
            <View style={styles.cardImage}>
              ...
              <View style={styles.imageBadges}>
                <View style={[styles.statusBadge, { backgroundColor: item.is_open ? colors.primary : colors.error }]}>
                  <Text style={styles.statusBadgeText}>{item.is_open ? 'Open Now' : 'Closed'}</Text>
                </View>
                {item.is_verified && (
...
const styles = StyleSheet.create({
  ...
  locationName: { fontSize: fontSize.md, fontWeight: fontWeight.bold },
  ...
  cardImage: { height: 200, position: 'relative' },
  ...
  verifiedBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: radius.full, flexDirection: 'row', alignItems: 'center' },
  ...
  priceRange: { fontSize: fontSize.sm, fontWeight: fontWeight.medium },
